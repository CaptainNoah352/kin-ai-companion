...
test("model policy defaults route Kin roles to the requested OpenRouter models", () => {
  assert.equal(getOpenRouterModelForRole(aiModelRoles.normalCoach), "google/gemini-3.1-flash-lite");
  ...
  assert.equal(getOpenRouterModelForRole(aiModelRoles.deepSupport), "openai/gpt-5.4-mini");
